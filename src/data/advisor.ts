...
export function buildAnswer(d: Dilemma, sel: AdvisorSelection): Answer {
  ...
  return {
    ...
    insight:
      "Сейчас BI Group продаёт не бизнес, а гипотезу о бизнесе — а гипотезы стоят на порядок дешевле долей. Правильный товар для партнёра сегодня — ограниченное право проверить канал на понятных условиях, а не участие в будущей стоимости, которую никто пока не может посчитать.",
    evidenceLevel,
...
export function buildFollowUpReply(question: string, answer: Answer): string {
  const q = question.toLowerCase();

  if (/объём|гаранти/.test(q)) {
    return `Гарантированный объём меняет расчёт риска: часть неопределённости, из-за которой вывод звучит осторожно, снимается. Это стоит закрепить как отдельное условие сделки — рядом с «${answer.terms[0] ?? "остальными предложенными условиями"}», а не как устный договор.`;
  }
  if (/эксклюзив|сегмент/.test(q)) {
    return `Сужение эксклюзивности до одного сегмента снижает объём прав, которые получает партнёр, и делает вывод «${answer.verdict}» менее рискованным. Логика рекомендации при этом не меняется — меняется только цена вопроса.`;
  }
  if (/сценар|окно|срок|месяц/.test(q)) {
    const recommended = answer.scenarios.find((s) => s.recommended);
    return `При таком горизонте по-прежнему сильнее всего смотрится сценарий «${recommended?.name ?? answer.scenarios[0]?.name ?? "рекомендованный вариант"}» — он и был рассчитан на короткое рыночное окно, а не на длинную дистанцию.`;
  }
  if (/риск/.test(q)) {
    return `Ключевой риск здесь: ${answer.risks[0] ?? "риск уже описан выше"}. Остальные факторы из списка рисков не отменяют вывод, но их стоит держать в поле зрения до подписания.`;
  }
  return `Это уточняет условия, но не меняет основной вывод: ${answer.verdict} Как только появятся точные цифры по этому пункту, рекомендацию стоит пересчитать заново, а не подгонять под уже сделанный вывод.`;
}
