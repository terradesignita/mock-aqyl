...
export const ru = {
  ...
  newCase: {
    ...
    progressNote: "Кейс появится в списке сразу, разбор пойдёт в фоне.",
    pendingSummary:
  ...
  /** Виджет статуса обработки файлов — рис. 41. */
  ingestWidget: {
    titleRunning: (n: number) => `Обрабатываем ${n} ${pluralRu(n, "файл", "файла", "файлов")}`,
    titleDone: (n: number) => `${n} ${pluralRu(n, "файл", "файла", "файлов")} готово`,
    titleError: (n: number) => `${n} ${pluralRu(n, "файл", "файла", "файлов")} не принято`,
    titleMixed: (done: number, failed: number) => `Готово ${done} · не принято ${failed}`,
    collapse: "Свернуть статус обработки",
    expand: "Развернуть статус обработки",
    close: "Убрать завершённые",
    dismissOne: "Убрать из списка",
    done: "Разобран и добавлен в кейс",
    open: "Открыть",
    rejected: (format: string) => `${format} — формат не поддерживается`,
    pickAnother: "Выбрать другой файл",
  },

  /** Единственная точка справки вместо 18 кнопок «?» по интерфейсу (§6 отчёта). */
  help: {
    open: "Как это работает",
    title: "Как это работает",
    subtitle:
      "Семь понятий, на которых держится платформа. Здесь то, что не удалось объяснить самой подписью на экране.",
    caseTitle: "Кейс и материал",
    caseBody:
      "Кейс — это папка по одной теме: внутрь кладутся материалы (отчёт, презентация, запись, статья). Материал — один документ внутри кейса. Кейс создаётся кнопкой «Новый кейс» из файла: название платформа берёт из содержимого, а не из имени файла.",
    sourcesTitle: "Источники и контекст",
    sourcesBody:
      "Ассистент отвечает только по источникам, отмеченным галочкой в левой панели. Снятая галочка выводит материал из контекста — и ответ меняется. Если не отмечено ничего, ассистент отказывается отвечать, а не придумывает. Сноски [1], [2] в ответе ведут к конкретным цитатам.",
    advisorTitle: "Поиск и AI-советник",
    advisorBody:
      "Поиск ищет по кейсам: названиям, описаниям и тегам. AI-советник нужен, когда надо принять решение: он задаёт уточняющие вопросы, показывает, как понял ситуацию, и выдаёт рекомендацию со сценариями, условиями и рисками.",
    evidenceTitle: "Доказательность и применимость",
    evidenceBody:
      "Доказательность — на чём стоит вывод: высокая — несколько независимых источников; средняя — один надёжный; низкая — косвенные данные; недостаточно данных — советник отказывается от рекомендации. Применимость — насколько похож кейс-аналог на вашу ситуацию: высокая, частичная или слабая аналогия.",
    artifactsTitle: "Артефакты",
    artifactsBody:
      "Готовые форматы на основе отмеченных источников: тест, презентация, отчёт, карточки, подкаст, инфографика. «⋮» на плитке — перегенерировать, открыть на весь экран или скачать. Цифры и цитаты в артефактах стоит сверять с исходником перед отправкой.",
    councilTitle: "Консилиум",
    councilBody:
      "Групповое обсуждение вашей задачи одновременно несколькими AI-персонами: вместо одного мнения вы получаете спор и разные точки зрения. Персоны — цифровые модели публично известных подходов, а не реальные люди и не их частные взгляды. Участников — от одного до трёх.",
    privacyTitle: "Приватные и общие кейсы",
    privacyBody:
      "Приватный кейс виден только вам и не участвует в общем поиске. Общий доступен всем сотрудникам. Переключается на самой карточке кейса.",
    shortcutsTitle: "Горячие клавиши",
    keySlash: "/",
    keySlashWhat: "курсор в поиск",
    keyEsc: "Esc",
    keyEscWhat: "закрыть окно",
    keyF: "F",
    keyFWhat: "артефакт во весь экран",
  },
