    ...
    <div className="space-y-3">
      ...
      <section className="rounded-card border-2 border-primary/40 bg-card p-5 shadow-soft">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-2">
          <span
            className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-bold uppercase tracking-wide ${EVIDENCE_TONE[answer.evidenceLevel]}`}
          >
      ...
      <div className="grid items-start gap-3 xl:grid-cols-2">
        ...
        <Section title={`Релевантный кейс: ${answer.caseRef.title}`} icon={BookOpen} defaultOpen>
          <span
            className={`inline-flex rounded-full border px-2.5 py-0.5 text-xs font-bold uppercase tracking-wide ${APPLICABILITY_TONE[answer.caseRef.applicability]}`}
          >
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <div className="rounded-control border border-scope-internal/40 bg-scope-internal/8 p-3">
              <p className="text-xs font-bold uppercase tracking-wider text-scope-internal">
                Что совпадает
              </p>
        ...
        <Section title="Рекомендация и предлагаемые условия" icon={Target} defaultOpen>
          ...
          <p className="mb-1.5 mt-3 text-xs font-bold uppercase tracking-wider text-muted-foreground">
              ...
              {answer.scenarios.map((s) => (
                ...
                >
                  <td className="py-2 pr-3 font-semibold text-card-foreground">
                    {s.name}
                    {s.recommended && (
                      <span className="ml-2 rounded-full bg-primary px-2 py-0.5 text-xs font-bold uppercase text-primary-foreground">
                        рекомендуем
                      </span>
                    )}
                  </td>
      ...
      <div className="grid items-start gap-3 xl:grid-cols-2">
        ...
        <Section title={`Источники (${answer.sources.length})`} icon={Quote} defaultOpen>
            ...
            {answer.sources.map((s) => (
              <li key={s.id} className="rounded-control border border-border bg-secondary/40 p-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full bg-card px-2 py-0.5 text-xs font-bold uppercase tracking-wide text-muted-foreground">
                    {s.kind}
                  </span>
          ...
          <Link
            to="/card/$id"
            params={{ id: "card_001" }}
            className="mt-3 inline-flex items-center gap-1 text-xs font-bold uppercase tracking-wide text-primary transition-[gap] hover:gap-2"
          >
